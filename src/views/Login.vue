...
<template>
  ...
  <form @submit.prevent="login" class="form-fields">
    <label>Email</label>
    <input type="email" v-model="form.email" required placeholder="Enter email">

    <label>Password</label>
    <input type="password" v-model="form.password" required placeholder="Enter password">

    <button type="submit" class="btn-primary" :disabled="auth.isLoading">{{ auth.isLoading ? 'Logging in...' : 'Login' }}</button>
